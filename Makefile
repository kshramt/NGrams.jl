...
files := $(addprefix $(d)/,$(shell git ls-files))
jl_src_files := $(filter $(d)/src/%.jl,$(files))
jl_test_files := $(filter $(d)/test/%.jl,$(files))


.PHONY: check
check: $(d)/test/runtests.jl.tested


$(d)/test/runtests.jl.tested: $(d)/test/runtests.jl $(jl_test_files) $(jl_src_files)
	$(JULIA) $<
	touch $@
